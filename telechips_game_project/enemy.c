    ...
    for (int i = 0; i < ENEMIES_N; i++)
    {
        ...
        case ENEMY_TYPE_2: // 일반 몬스터 2
            if (frames % 2) {
                ...
                // y축 경계에서 반전
                if (enemies[i].y < 0.4 * BUFFER_H || enemies[i].y > 0.78 * (BUFFER_H - ENEMY_H[1]))
                    enemies[i].vy *= -1;
        ...
        case BOSS_TYPE_2: // 보스 몬스터 //이것도 BOSS1이랑 같게 추가
            if (frames % 2) // 속도 보정
            {
                ...
                // 경계 체크 (화면 밖으로 못 나가게 제한)
                int boss_w = ENEMY_W[BOSS_TYPE_2];
                int boss_h = ENEMY_H[BOSS_TYPE_2];
        ...
        if (enemies[i].shot_timer <= 0) {
            ...
            if (enemies[i].type == BOSS_TYPE_1) {
                ...
                shots_add(false, true, cx, cy, DIR_UP, 10, ATTACK_BOSS1);
                shots_add(false, true, cx, cy, DIR_DOWN, 10, ATTACK_BOSS1);

                enemies[i].shot_timer = (int)(60 * fire_mult[game_difficulty - 1]);
            }
            else if (enemies[i].type == BOSS_TYPE_2) { //boss1이랑 같게 추가 필요하면 수정
                // 보스: 상하좌우 4방향 발사
                shots_add(false, true, cx, cy, DIR_LEFT, 10, ATTACK_BOSS2);
                shots_add(false, true, cx, cy, DIR_RIGHT, 10, ATTACK_BOSS2);
                shots_add(false, true, cx, cy, DIR_UP, 10, ATTACK_BOSS2);
                shots_add(false, true, cx, cy, DIR_DOWN, 10, ATTACK_BOSS2);
...
void enemies_draw()
{
    for (int i = 0; i < ENEMIES_N; i++) {
        ...
        if (enemies[i].type == BOSS_TYPE_1) {
            ...
            //플레이어가 오른쪽 있을 때
            int face_right = (player_cx > enemy_cx);  
            ...
            if (!face_right) {
                ...
                al_draw_scaled_bitmap(
                    sprites.enemy[enemies[i].type],
                    0, 0, w, h,
                    enemies[i].x + draw_w, enemies[i].y,  
                    -draw_w, draw_h,                      
                    0
                );
            }
